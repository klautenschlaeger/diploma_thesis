...
The mean latency is nearly the same as in \autoref{fig:mean_50}, which indicates, that their is still some channel capacity left for
further data exchange.

Both simulation were based on the abstracted and simplified corn harvest scenario, which is based on the ns-3 communication model and
abstracted implementation of the physical layer and medium access control layer in ns-3.
In a real-world scenario, an Agricultural Platooning Service has to operate in the complex agricultural environment, which
may impact the signal propagation and communication performance.
...
These physical layer amendment is designed to introduce a new level of long range communication in IEEE 802.11.
At the moment, I am not aware of any research, which was able to measure the resulting communication range in a agricultural environment.

\todo{Field planner, Cellular, RSU }
