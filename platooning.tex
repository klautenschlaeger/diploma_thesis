...
the TableBasedErrorRateModel derives the \ac{PER} from results form the Matlab WLAN Toolbox in regards to the physical layer parameters and \ac{SNR}.
...
Each corn harvest scenario is visualized to ensure the correctness of the simulation. The visualizations are created with the
ns-3 extension Netsimulyzer.
The corn field is marked as an black rectangle.
At the lower left corner there is a cuboid which marks the location of the silo.
Every node was displayed as the Netsimulyzer model of an landdrone, where all \ac{FH}s are black and the
\ac{TM}s are either red, green, yellow or blue
