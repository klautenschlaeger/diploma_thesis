...
\begin{table}[!ht]
   ...
   \begin{tabular}{>{\raggedright}p{2cm}p{3cm}p{2cm}}
      ...
      \bottomrule
   \end{tabular}
   \caption{\ac{MCS} and \ac{CR} for HE-\ac{MCS} values \cite{ieee_standard_2021ax}}
   \label{tab:HEMCS}
\end{table}
...
\begin{figure}%
	...
	\caption{Receiver minimum input level sensitivity for different HE-MCS values according to \cite{ieee_standard_2021ax}, where \ac{PER} is less than \SI{10}{\percent}}%
...
HE-\ac{MCS}-\num{1}, HE-\ac{MCS}-\num{3} and HE-\ac{MCS}-\num{4} for \num{1} or \num{2} spatial
transmission streams \cite{ieee_standard_2021ax}.
...
MU-\ac{MIMO} DCM can also be applied in IEEE 802.11ax \cite{ieee_standard_2021ax}.
