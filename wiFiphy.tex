...
\subsubsection*{\acf{BW}} \todo[color=blue]{same as Channel width bandwith?}
Üm mehrere Übertragungskanäle zu schaffen, wurden die Frequenzbereiche in Kanäle aufgeteilt.
Damit ergeben sich in Europa 13 Kanäle im Frequenzbereich von \SIrange{2.412}{2.482}{\giga\hertz} und 18 Kanäle im Frequenzbereich von \SIrange{5.180}I{5.350}{\giga\hertz} und \SIrange{5.180}I{5.350}{\giga\hertz} \cite{sauter_wireless_2022}.
To enable higher data rates, transmission channels can be combined. This is called cahnnel \ac{BW} and is specified in \ac{MHz}.
The possible \ac{BW}s for the IEEE 802.11 standards are shown in Table \ref{tab:BW}. \todo[color=blue]{How to cite table? how to make it nicer?}
\begin{table}[!ht]
	\centering
	\begin{tabular}{>{\raggedright}p{1.8cm}p{2.7cm}p{2.7cm}p{2.7cm}}
		\toprule
		Standard & Channel \ac{BW}s\newline in \SI{2.4}{\giga\hertz}\newline Frequenzy range & Channels in  \SI{5}{\giga\hertz} & Channels in \SI{5.9}{\giga\hertz}\\
		\midrule
		IEEE 802.11n & \SI{20}{\mega\hertz},\SI{40}{\mega\hertz}  & \SI{20}{\mega\hertz},\SI{40}{\mega\hertz} & - \\
		IEEE 802.11ac & -  & \SI{20}{\mega\hertz},\SI{40}{\mega\hertz}, \SI{80}{\mega\hertz},\SI{160}{\mega\hertz} & - \\
		IEEE 802.11ax &\SI{20}{\mega\hertz},\SI{40}{\mega\hertz}  & \SI{20}{\mega\hertz},\SI{40}{\mega\hertz}, \SI{80}{\mega\hertz},\SI{160}{\mega\hertz} & - \\
		IEEE 802.11p & - & - & \SI{10}{\mega\hertz} \\
		IEEE 802.11bd & - & - & \SI{10}{\mega\hertz},\SI{20}{\mega\hertz} \\
		\bottomrule
	\end{tabular}
	\caption{\cite{jacob_system-level_2020}, \cite{sauter_wireless_2022}, \cite{noauthor_ieee_2021}}
	\label{tab:BW}
\end{table}
...
While wider channels increase the theoretical data rate, \textcite{Avallone} mentions, that narrower channels can
increase the power spectral density of the signal and thus increase the transmission range.
...
the roll-off factor as a cosine-shaped flattening of the square signal between 0 and 1. In addition, the author points
...
the following conditions \cite{afaqui_ieee_2017} \cite{noauthor_ieee_2021}:
\begin{itemize}
...
IEEE 802.11ax \cite{jacob_system-level_2020} \cite{krief_analysis_2020}.
...
Together with the \ac{MCS}, the \ac{FEC} \ac{CR} form a physical layer specification. This is named after the standard
and includes the possible expressions for \ac{MCS} values and \ac{CR} of the standard. For IEEE 802.11ax,
this results in the HE-MCS index values in \autoref{tab:HEMCS}.
...
At the receiver, the data copies are combined with the log-likelihood ratio. Thus \ac{DCM} increases the probability of
receiving the data.

\cite{noauthor_ieee_2021} provides a receiver minimum input sensitivity, which indicates until which RSS a packet is
received with a probability of \SI{90}{\percent}. The receiver minimum input sensitivity for a \ac{BW} of
\SI{20}{\mega\hertz} is displayed in \autoref{fig:ReceiverSensitivityDCM}. It demonstrates that when using \ac{DCM},
...
The higher probability of achieving data is achieved at the expense of the data rate. The same amount of data now takes
twice as long to transmit.

\cite{noauthor_ieee_2021} lists the theoretically possible data rates. These reveal that the maximum achievable data
rate with DCM is only half of the attainable data rate without DCM.
