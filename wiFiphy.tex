...
IEEE 802.11ax stations must support \ac{LDPC} when using on the IEEE 802.11ax standard under the following conditions \cite{afaqui_ieee_2017} \cite{standard_ieee_2021}:
\begin{itemize}
...
\end{itemize}
IEEE 802.11ax achieves \ac{CR} of \nicefrac{1}{2}, \nicefrac{2}{3}, \nicefrac{3}{4}, and \nicefrac{5}{6} \cite{standard_ieee_2021}.
...
\begin{comment}
	...
	optional feature \cite{standard_ieee_2021}
	
	HE SU PPDU HE ER SU PPDU not for GI 800 ns \cite{standard_ieee_2021}
\end{comment}
...
\todo{bd angenommen? Wie komme ich an den Standard?}
...
The authors explain that since IEEE 802.11n it is possible to use up to four MIMO streams. This number was increased again to up to eight MIMO streams in IEEE 802.11ax \cite{standard_ieee_2021}. Since data can be sent simultaneously via each MIMO stream, the theoretical data rate can thus increase proportionally depending on the usable streams. 
\begin{comment}
...
\textcite{abbas} sagt zudem, dass MIMO dazu genutzt werden kann, um die Qualität des empfangenen Signals zu verstärken.
Das geschieht in Form von spatial diversity und \ac{STBC}.
...
signal quality and increase the data rate. This results in a more reliable and efficient wireless communication system,
with improved data transfer speed and reduced error rates.
...
\cite{sauter_wireless_2022} Reichweite und throughput higher wegen better SNR in Range 


IEEE 802.11ax stations use \ac{STBC} when applying on the IEEE 802.11ax standard under the following conditions \cite{standard_ieee_2021}:
\begin{itemize}
