...
The router on the tractor sets up a Wi-Fi \ac{AP}. One of the boards on the \ac{CH} connects to the \ac{AP} of the router as a Wi-Fi \ac{STA} and hosts an iperf3 \footnote{https://iperf.fr/} server.
...
The other UP Squared Board on the \ac{CH} uses the Wi-Fi card in the monitor mode and records every transmission in the \SI{5.6}{\giga\hertz} band using tcpdump \footnote{https://www.tcpdump.org/}.
...
The tcpdump records are in pcap - format, which can be analyzed using Wireshark\footnote{https://www.wireshark.org/}.
