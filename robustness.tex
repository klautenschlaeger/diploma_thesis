...
called gr-ieee802-11 \footnote{https://github.com/bastibl/gr-ieee802-11 Accessed: 14.03.2023}\todo[color=blue]{Accessed necessary? }. However, the gr-ieee802-11 only
...
\end{itemize}
\todo[color=blue]{use table? Other format?}
...
Taking the Friis transmission model \cite{shaw_radiometry_2013} into account, the RSSI for omnidirectional antennas can be calculated in regard to the distance $d$,
receiving antenna gain $G_{R}$, transmitting antenna gain $G_{T}$ and transmission power $P_{T}$  as follows:
\begin{equation}
	RSSI = P_{T} * G_{T} * G_{R} * \frac{\lambda^2}{(4 * \pi * d)^2}.
\end{equation}
When $\lambda$ is replaced and  $P_{T}$,  $G_{T}$,  and $G_{R}$ are set to \num{1} the following equation can be derived:
\begin{equation}
	RSSI = \frac{c^2}{(f* 4 * \pi * d)^2},
\end{equation}
...
\end{equation}
The equation shows, that a lower frequency results in a higher \ac{RSSI} for a given distance.
